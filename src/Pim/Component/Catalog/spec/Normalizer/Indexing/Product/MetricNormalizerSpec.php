...
class MetricNormalizerSpec extends ObjectBehavior
{
    ...
    function it_support_metrics(MetricInterface $metric)
    {
        ...
        $this->supportsNormalization($metric, 'indexing')->shouldReturn(true);
    }

    function it_normalizes_product_assocations($stdNormalizer, MetricInterface $metric)
    {
        $stdNormalizer->normalize($metric, 'indexing', ['context'])->willReturn(['std-metric']);
        ...
        $this->normalize($metric, 'indexing', ['context'])->shouldReturn(
